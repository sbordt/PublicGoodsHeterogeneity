...
def gfe(Y, X, XHet, N, T, G, init = gfe_alg_1_default_init, maxIter = 1000, hit = 5, feasible = lambda beta, betaHet, alpha: True):
    """ Grouped Fixed-Effects

        feasible(beta, betaHet, alpha) returns True/False and can be used to restict the set of feasible soluions. All soluions
        for which feasible(...) == False will be silently ignored. NOTE: There is no such thing as soluions at the "feasibility border".
    """
    # find a fist feasible solution
    [beta, betaHet, alpha, fun, ass] = gfe_alg_1(Y, X, XHet, N, T, G, init = init)
    nconsecutive = 1
    itercount = 1

    while not feasible(beta, betaHet, alpha):
        [beta, betaHet, alpha, fun, ass] = gfe_alg_1(Y, X, XHet, N, T, G, init = init)

        itercount = itercount+1
        
        if itercount == maxIter:
            raise Exception('gfe: no feasible solution found withing maxIter iterations')
            break
    
    # run algorithm 1 until we found the same smallest value 'hit' consecutive times or we reach maxIter    
    while(True):
        [beta1, betaHet1, alpha1, fun1, ass1] = gfe_alg_1(Y, X, XHet, N, T, G, init = init)

        if feasible(beta1, betaHet1, alpha1):
            if abs(fun-fun1)/fun < 1e-6:   # another time
                ...
                if nconsecutive % 5 == 0:
                    print "Hit %d times." % nconsecutive
    ...
    return [beta, betaHet, alpha, fun, ass]
